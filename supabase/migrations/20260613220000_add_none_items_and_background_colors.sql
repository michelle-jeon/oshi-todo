...
upserted as (
  ...
  select
    code, name, slot, species, 0, payload, thumbnail_url, true, true, true,
    'gem', 0, 1, sort_order
  from optional_items
...
from upserted
join optional_items using (code)
...
upserted as (
  ...
  select
    code, name, 'background'::public.inventory_item_slot, null, 0, payload, thumbnail_url, true, true, true,
    'gem', 0, 1, sort_order
